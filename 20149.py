...
def main():
    ...
    if x1 == x2 and x3 != x4:
        if x3 <= x1 <= x4:
            if y1 <= yd2 / xd2 * (x1 - x3) + y3 <= y2: # 겹치는 좌표 존재
                pFlag = 1
                ans = 1
                crossX = x1
                crossY = (yd2 / xd2) * (crossX - x3) + y3
            else:
                ans = 0
        else:
            ans = 0
        ...
        if yd1 * xd2 == yd2 * xd1: # 기울기 같음
            ...
            if x1 <= x3 <= x2 or x1 <= x4 <= x2 or x3 <= x1 <= x4 or x3 <= x2 <= x4: #범위 겹침
                ...
                flag1, flag2 = 0, 0
                pFlag = 1
                crossX = (yd1 * xd2 * x1 - yd2 * xd1 * x3 + xd1 * xd2 * (y3 - y1)) / (yd1 * xd2 - yd2 * xd1)
                crossY = (yd1 / xd1) * (crossX - x1) + y1
