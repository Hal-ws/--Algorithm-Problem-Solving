...
def main():
    ...
    visit = [[[[0 for i in range(10)] for j in range(10)] for k in range(10)] for l in range(10)]
        ...
        if r != None and b != None: #두 공이 다 있을때 다음 횟수 고려
            ...
            if dir != 2: #상으로 갈 수 있을 때
                if ry <= by:
                    ...
                    nr = up(r, nb)
                if nr != r or nb != b:
                    if nr == None or nb == None:
                        q.append([nr, nb, cnt + 1, 0])
                    else:
                        if visit[nr[0]][nr[1]][nb[0]][nb[1]] == 0:
                            q.append([nr, nb, cnt + 1, 0])
                            visit[nr[0]][nr[1]][nb[0]][nb[1]] = 1
            if dir != 3: #오른쪽으로 이동가능
