...
def main():
    ...
    for _ in range(T):
        ...
        l1 = sorted([[tmp[0], tmp[1]], [tmp[2], tmp[3]]])
        box = [[tmp[4], tmp[5]], [tmp[6], tmp[7]]]
        box.append([box[1][0], box[0][1]])
        box.append([box[0][0], box[1][1]])
        box.sort()
        if crossChk(l1, [box[0], box[1]]):
            print('T')
            continue
        if crossChk(l1, [box[0], box[2]]):
            print('T')
            continue
        if crossChk(l1, [box[2], box[3]]):
            print('T')
            continue
        if crossChk(l1, [box[1], box[3]]):
            print('T')
            continue
        flag = 0
        for p in l1:
            x, y = p[0], p[1]
            if (box[0][1] <= y <= box[1][1]) and (box[0][0] <= x <= box[2][0]):
                print('T')
                flag = 1
            if flag:
                break
